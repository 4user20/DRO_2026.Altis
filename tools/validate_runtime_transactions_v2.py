...
REQUIRED: dict[str, tuple[tuple[str, ...], ...]] = {
    ...
    "dro2026/functions/directors/fn_longRangeDroneDirector.sqf": (
        ("DRO2026_fnc_isLiveContactSubject", "_isLiveStrategicContact"),
...
def main() -> int:
    ...
    for relative, alternatives in REQUIRED.items():
        path = ROOT / relative
        if not path.is_file():
        ...
        for group in alternatives:
            if not any(snippet in source for snippet in group):
                errors.append(
                    f"{relative}: missing one of {', '.join(repr(item) for item in group)}"
                )
