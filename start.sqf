...
fnc_selectObjective = compile preprocessFile "dro2026\objectives\selectObjective.sqf";
...
[] call DRO2026_fnc_sanitizeLegacyPools;
	...
	_thisCategory = _x;
	{
		...
		if (
			...
			{			
				...
				if !([_vehicle, playersSide] call DRO2026_fnc_isSafeInfantryClass) then {_save = false};
...
_startingLoadoutGroup = _startingLoadoutGroup select {[_x, playersSide] call DRO2026_fnc_isSafeInfantryClass};
...
// Mission name
_missionName = format ["Операция «%1»", selectRandom ["Разрыв цепи", "Тихий контур", "Выжженный маршрут", "Слепая зона", "Короткое окно", "Перехват", "Глубокий след", "Контрбатарея"]];
...
// DRO2026 EARLY DYNAMIC: distant generated groups sleep until tactically relevant.
{if ((side _x) == enemySide) then {_x enableDynamicSimulation true}} forEach allGroups;
...
if (false && {stealthEnabled == 1}) then {
...
[] call DRO2026_fnc_startDirectors;
...
if (false && {((AOLocations select 0) select 4) == 0}) then {
