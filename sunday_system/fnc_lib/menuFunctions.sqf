...
sun_clearInsert = {
	...
	{
		[626262, 6006, "Точка высадки: СЛУЧАЙНО"] remoteExec ["sun_lobbyChangeLabel", _x];	
	} forEach allPlayers;
};
...
dro_menuMap = {
	...
	*/
	if (isNil "mapOpen") then {
		...
		_text = composeText ["Выберите район операции:", lineBreak, lineBreak, "Щёлкните по карте, чтобы выбрать ближайший район.", lineBreak, "ALT + щелчок задаёт точную пользовательскую позицию."];
			...
			"onMapSingleClick",
			{
				...
				if (_alt) then {
					...
					if (isNil "_nearLoc") then {
						aoName = format ["Сельская местность — %1", ((configfile >> "CfgWorlds" >> worldName >> "description") call BIS_fnc_getCfgData)];
					} else {
				...
				((findDisplay 52525) displayCtrl 2010) ctrlSetText format ["Район операции: %1", aoName];
	...
	*/
	switch (_preset) do {
		case 1: {					
			...
			sliderSetPosition [2041, 1*10];
			((findDisplay 52525) displayCtrl 2040) ctrlSetText "Множитель сил противника: x1.0";				
			["MAIN", 2050, 0] call sun_switchButtonSet;
			["MAIN", 2060, 0] call sun_switchButtonSet;
			["MAIN", 2070, 0] call sun_switchButtonSet;
			["MAIN", 4010, 0] call sun_switchButtonSet;				
		...
		case 2: {					
			...
			((findDisplay 52525) displayCtrl 2040) ctrlSetText "Множитель сил противника: x0.5";	
			["MAIN", 2050, 0] call sun_switchButtonSet;			
			["MAIN", 2060, 0] call sun_switchButtonSet;					
		...
		case 3: {					
			...
			((findDisplay 52525) displayCtrl 2040) ctrlSetText "Множитель сил противника: x1.25";	
			["MAIN", 2050, 0] call sun_switchButtonSet;
			["MAIN", 2060, 0] call sun_switchButtonSet;
			["MAIN", 2070, 0] call sun_switchButtonSet;
			["MAIN", 4010, 0] call sun_switchButtonSet;				
