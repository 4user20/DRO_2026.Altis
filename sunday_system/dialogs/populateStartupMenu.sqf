...
/*
_index = lbAdd [2103, "Случайно"];
_index = lbAdd [2103, "Рассвет"];
_index = lbAdd [2103, "День"];
_index = lbAdd [2103, "Сумерки"];
_index = lbAdd [2103, "Ночь"];
*/
lbAdd [2104, "Случайно"];
lbAdd [2104, "Январь"];
lbAdd [2104, "Февраль"];
...
/*
lbAdd [2116, "Случайно"];
lbAdd [2116, "Настроить"];
*/
_index = lbAdd [2106, "Случайно"];
_index = lbAdd [2106, "1"];
_index = lbAdd [2106, "2"];
_index = lbAdd [2106, "3"];
...
((findDisplay 52525) displayCtrl 2041) ctrlSetText format ["Множитель сил противника: x%1", profileNamespace getVariable ['DRO_aiMultiplier', 1]];
...
if (!isNil "aoName") then {
	ctrlSetText [2202, format ["Район операции: %1", aoName]];
};
...
_tooltip = str composeText [	 
	"AI Skill",
	toString [13, 10],
	"Обычный: заметно снижает точность AI, сохраняя тактические навыки. Рекомендуемый баланс для процедурной операции.",
	toString [13, 10],
	"Сложный: умеренно снижает точность AI и сохраняет его тактические навыки.",
	toString [13, 10],
	"Реализм: не изменяет навыки AI и использует параметры сложности Arma.",
	toString [13, 10]
];
