...
if (count _targetASL < 2) exitWith {_drone setVariable ["dro2026_fpvInitialized", false]; "NO_TARGET"};
...
private _applyFlightVector = {
    ...
    // Orientation first: setVelocity is applied in the resulting flight direction.
...
while {
    ...
    if (!isNull _target && {!alive _target}) exitWith {_result = "TARGET_LOST"};

    if (!isNull _target) then {
        private _targetVelocity = velocity _target;
        private _distanceToTarget = (getPosASL _drone) distance2D (getPosASL _target);
        private _leadTime = linearConversion [0, 1200, _distanceToTarget, 0.18, 1.15, true];
        _targetASL = (getPosASL _target) vectorAdd (_targetVelocity vectorMultiply _leadTime);
    };

    private _droneASL = getPosASL _drone;
    private _targetAGL = ASLToAGL _targetASL;
    private _distance2D = _droneASL distance2D _targetASL;
    private _distance3D = vectorMagnitude (_targetASL vectorDiff _droneASL);
    private _tick = if (_distance2D > 500) then {0.28} else {if (_distance2D > 120) then {0.18} else {0.10}};
    ...
    if ((time - _lastProgressCheck) >= 8) then {
        ...
        if (_progress < (4 max (_lastDistance * 0.02))) then {
            ...
            private _currentAGL = ASLToAGL _droneASL;
            private _escapeAGL = _currentAGL getPos [70, (getDir _drone) + selectRandom [-55, 55]];
            _escapeAGL set [2, ((_currentAGL select 2) + 22) max 28];
            _recoveryASL = AGLToASL _escapeAGL;
            _recoveryUntil = time + 4.5;
        } else {
            _stuckCount = 0;
        };
    ...
    if (_stuckCount >= 4) exitWith {_result = "STUCK"};

    if (time > _guidanceLostUntil) then {
        private _aimASL = [];
        if (time < _recoveryUntil && {count _recoveryASL > 1}) then {
            _aimASL = +_recoveryASL;
        } else {
            ...
            // calculateTerrainAwareAim explicitly consumes AGL and returns ASL.
            _aimASL = [_drone, _targetAGL, _attackHeight, [70, 140, 240], 260, _lateralNoise] call DRO2026_fnc_calculateTerrainAwareAim;
        ...
        private _delta = _aimASL vectorDiff _droneASL;
        ...
        if (_deltaLength > 0.1) then {
            ...
            private _blend = if (_turnAngle > 0.01) then {(_maxTurnStep / _turnAngle) min 0.48} else {1};
            ...
            if (_blendedLength > 0.01) then {
                ...
                private _desiredSpeed = if (_distance2D > 450) then {40} else {if (_distance2D > 90) then {34} else {29}};
                private _newSpeed = _currentSpeed + ((_desiredSpeed - _currentSpeed) * (0.18 + 0.20 * _channelQuality));
...
if (_result == "ABORTED" && {time >= _timeout}) then {_result = "TIMEOUT"};
