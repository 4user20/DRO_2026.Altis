...
while {!(missionNamespace getVariable ["DRO2026_missionEnding", false])} do {
    ...
    if (count _known > 0 && {DRO2026_alertLevel > 0.48}) then {
        ...
        if (count _nonStatic < 2 && {DRO2026_alertLevel > 0.72} && {DRO2026_fpsAverage >= DRO2026_MIN_FPS_FOR_REINFORCEMENTS} && {(DRO2026_resources getOrDefault ["enemyReinforcement", 0]) >= 10} && {DRO2026_reinforcementWaves < DRO2026_MAX_REINFORCEMENT_WAVES}) then {
            ...
            private _grp = [_spawn, 3, 4, 100, false] call DRO2026_fnc_spawnGuard;
            if (!isNull _grp) then {_grp setVariable ["DRO2026_static", false]; private _wp = _grp addWaypoint [_targetPos getPos [450, random 360], 50]; _wp setWaypointType "MOVE"; _wp setWaypointSpeed "FULL"; private _wp2 = _grp addWaypoint [_targetPos, 80]; _wp2 setWaypointType "SAD"; DRO2026_resources set ["enemyReinforcement", ((DRO2026_resources getOrDefault ["enemyReinforcement", 0]) - 10) max 0]; DRO2026_reinforcementWaves = DRO2026_reinforcementWaves + 1};
        };
    };
