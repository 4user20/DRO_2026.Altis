    ...
    if ((time - DRO2026_lastEnemyFPV) >= _cooldown && {DRO2026_alertLevel > 0.35} && {_stock > 0} && {_slots > 0}) then {
        ...
        if (count _contacts > 0) then {
            ...
            private _sites = DRO2026_sites select {
                (_x getOrDefault ["networkNodeId", ""]) == "NODE_FPV_FORWARD_01" || {(_x getOrDefault ["type", ""]) == "FPV_TEAM"}
            };
            ...
            if (count _sites > 0) then {
                ...
                ["DRONE_LAUNCHED", createHashMapFromArray [["role", "FPV"], ["count", _count], ["contactId", _contact getOrDefault ["id", ""]]], "NODE_FPV_FORWARD_01"] call DRO2026_fnc_emitEvent;
                ...
                [format ["FPV-node запустил %1 аппарат(а), остаток комплектов %2", _count, (_kits - _count) max 0]] call DRO2026_fnc_log;
            };
        };
    };
};