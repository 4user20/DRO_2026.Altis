...
private _targetPosition = _contact getOrDefault ["position", _position];
...
[_origin, _contact, _operator, _manualControl, _launchCount, _requestSide, _requester] spawn {
    params ["_origin", "_contact", "_operator", "_manualControl", "_count", "_requestSide", "_requester"];
    for "_index" from 0 to (_count - 1) do {
        private _launchOrigin = _origin getPos [4 + random 10, random 360];
        [_launchOrigin, _contact, _requestSide, _operator, _manualControl, _requester] spawn DRO2026_fnc_launchFPVStrike;
