...
if (!isNull _requester && {side (group _requester) != playersSide}) exitWith {["Штаб: дальняя поддержка недоступна для этой стороны.", _requester] call DRO2026_fnc_supportMessage};
if !(_requestedType isEqualType "" && {_decoy isEqualType true} && {_quantity isEqualType 0}) exitWith {["Штаб: некорректный профиль дальнего удара.", _requester] call DRO2026_fnc_supportMessage};
...
if (_exactClass == "" && {!(_requestUpper in _knownTypes)}) exitWith {[format ["Штаб: неизвестный профиль дальнего удара %1.", _requestedType], _requester] call DRO2026_fnc_supportMessage};
if (_decoy && {_requestUpper != "AUTO"}) exitWith {["Штаб: режим обманки разрешён только для автоматического профиля.", _requester] call DRO2026_fnc_supportMessage};
...
if (_requestUpper == "FP5" && {DRO2026_friendlyFP5Used >= 2}) exitWith {["Штаб: лимит успешных запусков FP-5 для этой миссии исчерпан.", _requester] call DRO2026_fnc_supportMessage};
...
if (_launchCount <= 0) exitWith {["Штаб: Нет свободных аппаратов или достигнут лимит активных БПЛА.", _requester] call DRO2026_fnc_supportMessage};
...
if (count _sites == 0) exitWith {["Штаб: Дальний расчёт БПЛА не отвечает или площадка недоступна.", _requester] call DRO2026_fnc_supportMessage};
...
private _baseContact = ["PLAYER",objNull,_position,0.76,"НАЗНАЧЕННАЯ_ТОЧКА","","PLAYER_DESIGNATION",120,"",0,-1,createHashMapFromArray [["positionSpace","ASL"]]] call DRO2026_fnc_createContactRecord;
if (count _baseContact == 0) exitWith {["Штаб: не удалось сформировать запись назначенной цели. Ресурс не списан.", _requester] call DRO2026_fnc_supportMessage};
private _contacts = DRO2026_contacts select {
    ...
    {private _subjectId = _x getOrDefault ["subjectId", ""]; _subjectId == "" || {[_x] call DRO2026_fnc_isLiveContactSubject}}
};
...
if (count _siteContacts > 0) then {
    ...
    _baseContact = ["PLAYER",_record getOrDefault ["object",objNull],_record getOrDefault ["position",_position],0.92,_record getOrDefault ["type","ЦЕЛЬ"],"","PLAYER_DESIGNATION",90,"",0,-1,createHashMapFromArray [["positionSpace","ATL"],["stableSubjectId",_subjectId]]] call DRO2026_fnc_createContactRecord;
};
if (count _baseContact == 0) exitWith {["Штаб: цель не прошла проверку контактной модели. Ресурс не списан.", _requester] call DRO2026_fnc_supportMessage};

DRO2026_lastLongSupportRequest = time;
DRO2026_resources set [_costPool, (_stock - _launchCount) max 0];
["DRONE_LAUNCH_RESERVED", createHashMapFromArray [["role", "LONG_RANGE"], ["type", _requestedType], ["count", _launchCount], ["decoy", _decoy], ["siteId", _siteId], ["contactId", _baseContact getOrDefault ["id", ""]]], _siteId] call DRO2026_fnc_emitEvent;
...
[_origin, _baseContact, _operator, _requestedType, _decoy, _launchCount, _requestSide, _costPool, _siteId] spawn {
    ...
    for "_index" from 0 to (_count - 1) do {
        ...
        private _contactInvalid = (toUpperANSI (_baseContact getOrDefault ["state","ACTIVE"])) in ["LOST","DESTROYED","INVALID","EXPIRED"] || {(_baseContact getOrDefault ["bdaState", "DETECTED"]) in ["PROBABLY_DESTROYED", "CONFIRMED_DESTROYED"]} || {!isNull _target && {!alive _target}} || {_subjectId != "" && {!([_baseContact] call DRO2026_fnc_isLiveContactSubject)}};
        ...
        if (_abort) exitWith {private _unlaunched = _count - _index; DRO2026_resources set [_costPool, (DRO2026_resources getOrDefault [_costPool, 0]) + _unlaunched]; [format ["Отменена не запущенная часть salvo %1: возвращено %2", _type, _unlaunched]] call DRO2026_fnc_log};
        ...
        if (toUpperANSI _type == "FP5") then {
            private _strategicTarget = createHashMapFromArray [
                ...
                ["object",_contact getOrDefault ["target",objNull]],["serious",true]
            ];
