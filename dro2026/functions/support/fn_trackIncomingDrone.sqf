...
if (!hasInterface) exitWith {
    if (isServer && {!isNull _drone}) then {
        [_drone, _label, _side] remoteExec ["DRO2026_fnc_trackIncomingDrone", -2, false];
    };
};
...
while {alive _drone && {!(missionNamespace getVariable ["DRO2026_missionEnding", false])}} do {
    ...
    if (_isEnemy && {_visible}) then {
        ...
        _marker setMarkerPosLocal getPosATL _drone;
    };
    uiSleep 0.45;
};
