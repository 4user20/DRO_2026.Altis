...
if (isNull _vehicle || {!([_vehicle, enemySide] call DRO2026_fnc_crewManagedVehicle)}) exitWith {deleteMarker _marker; [_AOIndex] call DRO2026_fnc_objectiveUAVTeam};
...
[_taskName, _critical] spawn {params ["_task", "_critical"]; waitUntil {sleep 3; ({!isNull _x && {alive _x}} count _critical) == 0 || {missionNamespace getVariable ["DRO2026_missionEnding", false]}}; if !(missionNamespace getVariable ["DRO2026_missionEnding", false]) then {DRO2026_resources set ["enemyEW", 0]; [_task, "TARGET_DESTROYED", [["enemyEW", -65]]] call DRO2026_fnc_completeObjective}};
[] spawn {waitUntil {sleep 1; missionNamespace getVariable ["playersReady", 0] == 1 || {missionNamespace getVariable ["DRO2026_missionEnding", false]}}; if !(missionNamespace getVariable ["DRO2026_missionEnding", false]) then {sleep 8; ["EW_TASK"] call DRO2026_fnc_hqVoice}};
_taskName
