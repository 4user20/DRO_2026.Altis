...
private _bunker = createVehicle ["Land_BagBunker_Tower_F", _position getPos [18, 55], [], 0, "NONE"];
private _tent = createVehicle ["Land_TentA_F", _position getPos [14, 215], [], 0, "NONE"];
...
private _template = [_type, _position, _side, direction _hq] call DRO2026_fnc_createSiteComponents;
private _objects = ([_hq, _bunker, _tent] + (_template getOrDefault ["objects",[]])) select {!isNull _x};
private _components = _template getOrDefault ["components",createHashMap];
_components set ["crew",units _group];
_components set ["guards",(units _group) - [_officer]];
_components set ["launchers",[_hq]];
private _extra = createHashMapFromArray [["officer", _officer], ["group", _group], ["side",_side], ["components",_components], ["background", true]];
