...

{
    ...
    private _newStatus = if (count _refs == 0) then {
        ...
        if (count _liveRefs == 0) then {"DESTROYED"} else {
            if (count _liveRefs < count _refs) then {"DEGRADED"} else {"ACTIVE"}
        }
    };
...

{
    ...
    if (_nodeSeen getOrDefault [_nodeId, false]) then {
        ...
        private _newStatus = if (_nodeHasLive getOrDefault [_nodeId, false]) then {"ACTIVE"} else {"DESTROYED"};
        _node set ["physicalRefs", _refs];
        _node set ["physicalState", if (_newStatus == "ACTIVE") then {"ACTIVE"} else {"DESTROYED"}];
