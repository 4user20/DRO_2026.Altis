...
if (count _safeClasses == 0) then {_safeClasses = if (playersSide == west) then {["B_Soldier_F", "B_Soldier_AR_F", "B_medic_F"]} else {["I_Soldier_F", "I_Soldier_AR_F", "I_medic_F"]}};
private _positions = [["FRIENDLY_FORWARD"] call DRO2026_fnc_getTheaterNode, ["FRIENDLY_REAR"] call DRO2026_fnc_getTheaterNode];
{
    ...
    createMarker [_marker, _pos]; _marker setMarkerShape "ICON"; _marker setMarkerType "b_hq"; _marker setMarkerColor _sideColor;
    ...
    private _objects = [];
    {
        ...
        _obj setDir ((_x select 2) + 180); _objects pushBack _obj;
    ...
    for "_u" from 1 to 4 do {private _unit = _group createUnit [selectRandom _safeClasses, _pos, [], 7, "FORM"]; _unit setUnitPos (selectRandom ["MIDDLE", "UP"])};
    [_group] call DRO2026_fnc_registerManagedGroup; _group setVariable ["DRO2026_static", true]; [_group, _pos, 55] call BIS_fnc_taskDefend;
