...
private _validVehicle = {
    ...
    private _blocked = ["spawner", "module", "logic", "dummy", "placeholder", "_root", "site_", "pook_sam", "azncontrol"];
...
private _roleClasses = {
    ...
    private _classes = [];
    {
        _classes append (DRO2026_assetRegistry getOrDefault [_x, []]);
    } forEach _roles;
...
if ("UAV" in _categories) then {
    ...
    private _isrRoles = [
        "PLAYER_ISR_UAV",
        format ["ISR_MICRO_%1", _sideSuffix],
        format ["ISR_TACTICAL_%1", _sideSuffix],
        format ["ISR_HALE_%1", _sideSuffix]
    ];
    {
        private _name = [_x] call _displayName;
    ...
    } forEach ([_isrRoles, true, true] call _roleClasses);
