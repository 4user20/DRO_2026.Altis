...
private _cancelled = 0;
{
    ...
    switch _status do {
        ...
        case "CANCELLED": {_cancelled = _cancelled + 1};
...
private _highQualityIntel = count (_playerContacts select {
    ...
    {(_x getOrDefault ["uncertaintyRadius", 9999]) <= 250} &&
...
private _cancelledDeliveries = _eventCounts getOrDefault ["DELIVERY_CANCELLED", 0];
...
private _droneRecoveries = _eventCounts getOrDefault ["DRONE_RECOVERED", 0];
private _droneLosses = _eventCounts getOrDefault ["DRONE_LOST", 0];
...
private _objectiveFailures = _eventCounts getOrDefault ["OBJECTIVE_MATERIALIZATION_FAILED", 0];
private _objectiveExhausted = _eventCounts getOrDefault ["OBJECTIVE_SELECTION_EXHAUSTED", 0];
...
private _nodeCount = count DRO2026_networkNodes;
private _score = round (
    ((1 - _networkHealth) * 55) +
    (((_destroyed + _cancelled) / (_nodeCount max 1)) * 20) +
...
createHashMapFromArray [
    ["schema", 2], ["version", DRO2026_VERSION],
    ...
    ["activeNodes", _active], ["degradedNodes", _degraded], ["destroyedNodes", _destroyed], ["cancelledNodes", _cancelled],
    ...
    ["deliveriesInterdicted", _interdicted], ["deliveriesCompleted", _delivered], ["deliveriesCancelled", _cancelledDeliveries],
    ["friendlyFireMissions", _friendlyFireMissions], ["airRequests", _airRequests], ["droneLaunchEvents", _droneLaunches],
    ["droneRecoveries", _droneRecoveries], ["droneLosses", _droneLosses],
    ...
    ["objectiveMaterializationFailures", _objectiveFailures], ["objectiveSelectionExhausted", _objectiveExhausted],
