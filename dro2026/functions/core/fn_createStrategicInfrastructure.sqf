...
private _spawnHQ = {
    ...
    for "_i" from 1 to 2 do {
        private _guard = _group createUnit [_officerClass, _pos getPos [8 + random 8, random 360], [], 3, "FORM"];
        _guard setSkill 0.56;
    };
    ...
    private _record = createHashMapFromArray [
        ["type", _type], ["position", _pos], ["object", _hq],
        ["objects", [_hq, _bunker, _tent]], ["officer", _officer],
        ["group", _group], ["background", true]
    ];
...
private _safeAAClass = {
    ...
    if ((_name find "pook_") == 0 || {(_name find "spawner") >= 0} || {(_name find "_root") >= 0}) then {
...
private _spawnLayeredAA = {
    ...
    if (_withLongRange) then {
        ...
        if (_longClass != "") then {_longRange = createVehicle [_longClass, _longPos, [], 0, "NONE"]};
    ...
    private _shortRange = if (_shortClass != "") then {createVehicle [_shortClass, _shortPos, [], 0, "NONE"]} else {objNull};
    ...
    if (count _objects > 0) then {
        ...
        private _record = createHashMapFromArray [
            ["type", _siteType],
            ["position", if (!isNull _longRange) then {_longPos} else {_shortPos}],
            ["object", _primary], ["radar", _radar], ["shorad", _shortRange],
            ["objects", _objects], ["background", true], ["networked", _withLongRange]
        ];
        DRO2026_sites pushBack _record;
...
// Full LR network belongs to the AIR_DEFENCE objective; background uses one light SHORAD per side.
...
private _spawnDroneSite = {
    ...
    private _record = createHashMapFromArray [
        ["type", _type], ["position", _position], ["object", _operator],
...
if (DRO2026_ENABLE_BACKGROUND_ARTILLERY && {(DRO2026_sites findIf {(_x getOrDefault ["type", ""]) == "ARTILLERY_SITE"}) < 0}) then {
    ...
    if (!isNull _artillery && {count getArtilleryAmmo [_artillery] > 0}) then {
        ...
        DRO2026_sites pushBack createHashMapFromArray [
            ["type", "ARTILLERY_SITE"], ["position", _position],
            ["object", _artillery], ["positions", _positions], ["background", true]
        ];
