...
if (_phase == "DEPLOYMENT" && {_elapsed >= 30}) then {_phase = "RECON"};
if (_phase == "RECON" && {_confirmedIntel >= 2 || {DRO2026_alertLevel >= 0.30}}) then {_phase = "SHAPING"};
if (_phase == "SHAPING" && {_interdictions > 0 || {_destroyed >= 1} || {_networkHealth <= 0.82}}) then {_phase = "DISRUPTION"};
if (_phase == "DISRUPTION" && {_destroyed >= 2 || {_networkHealth <= 0.68}}) then {_phase = "DEEP_STRIKE"};
if (_phase == "DEEP_STRIKE" && {DRO2026_alertLevel >= 0.55} && {_destroyed >= 3 || {_networkHealth <= 0.55}}) then {_phase = "COUNTERATTACK"};
if (_phase == "COUNTERATTACK" && {_destroyed >= 4 || {_networkHealth <= 0.42}}) then {_phase = "EXPLOITATION"};
...
if (_phase != _oldPhase) then {
    ...
    ["PHASE_CHANGED",createHashMapFromArray [
        ...
        ["confirmedIntel",_confirmedIntel],["interdictions",_interdictions]
