...
if (!isNil "pInfClasses") then {
    _guardClasses = pInfClasses select {[_x, playersSide] call DRO2026_fnc_isSafeInfantryClass};
};
if (count _guardClasses == 0) then {
    _guardClasses = switch (playersSide) do {
